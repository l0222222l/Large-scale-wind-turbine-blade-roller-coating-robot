...
% 不规则的曲面
xe = 1 + 0.03 * sin(2 * pi * t / total_time) ...        
       + 0.05 * cos(4 * pi * t / total_time) ...       
       + 0.03 * exp(-t/10);                         
...
for i = 1 :length(t)
    ...
    for j = 1:N
        if j > 1
            a_future = (fd - fe_future(j-1) - b * v_future - k * (x_future - xe(i))) / m; 
            v_future = v_future + a_future * time_step;
            x_future = x_future + v_future * time_step;
        end

        fe_future(j) = ke(i) * max(0, x_future - xe(i));

        phi_future = [-a_future, -v_future, -(x_future - xe(i))]; 
        for k_idx = 1:Nu
            if j >= k_idx
    ...
    a = (fd - fe - b * v - k * (x - xe(i))) / m;
    v = v + a * time_step;
    x = x + v * time_step;

    fe_impedance = ke(i) * max(0, x_impedance - xe(i)); 
    a_impedance = (fd - fe_impedance - b_impedance * v_impedance - k_impedance * (x_impedance - xe(i))) / m_impedance; 
...
hold on;
plot(t, xe, 'k', 'LineWidth', 1.5); 
hold on;
plot(t, x_impedance_data, 'g', 'LineWidth', 1.5);
xlabel('时间 (s)');
ylabel('位置 (m)');
ylim([0.7, 1.3]); 
legend('离散时间MPC', '参考位置', '阻抗控制');
grid on;
